...
//  NewsApp
//
//  Created by Nurmat Junusov. on 2/7/22.
...
struct GIFImage: UIViewRepresentable {
    ...
    init(data: Data, restartGif: Binding<Bool>) {
        self.data = data
        self.name = nil
        self._restartGif = restartGif
    }

    public init(name: String, restartGif: Binding<Bool>) {
        self.data = nil
        self.name = name
        self._restartGif = restartGif
    }
    ...
    func makeUIView(context: Context) -> UIGIFImage {
        let uiGifImage: UIGIFImage
        if let data = data {
            uiGifImage = UIGIFImage(data: data)
        } else {
            uiGifImage = UIGIFImage(name: name ?? "")
        }
        return uiGifImage
    }
    
    func updateUIView(_ uiView: UIGIFImage, context: Context) {
        guard self.restartGif else {
            return
        }
        if let data = data {
